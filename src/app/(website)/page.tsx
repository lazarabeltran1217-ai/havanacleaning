  ...
  return (
    <>
      ...
      <section className="min-h-screen bg-tobacco flex items-center justify-center relative overflow-hidden">
        ...
        <svg className="absolute left-0 bottom-0 w-[300px] md:w-[420px] h-auto opacity-[0.12] pointer-events-none" viewBox="0 0 500 1000" fill="none" xmlns="http://www.w3.org/2000/svg">
          {/* Trunk - curved, leaning slightly right */}
          <path d="M160 1000 Q150 750 170 600 Q185 480 200 380 Q210 310 220 260" stroke="#2D6A4F" strokeWidth="22" strokeLinecap="round" fill="none"/>
          {/* Trunk texture marks */}
          <path d="M158 900Q175 895 185 900M155 830Q172 825 182 830M157 760Q174 755 184 760M162 690Q179 685 189 690M168 620Q185 615 195 620M175 550Q192 545 202 550M182 480Q199 475 209 480M190 410Q207 405 217 410M197 350Q214 345 224 350M205 300Q220 295 230 300" stroke="#2D6A4F" strokeWidth="2" opacity="0.5"/>
          {/* Coconut cluster */}
          <circle cx="215" cy="268" r="10" fill="#2D6A4F"/>
          <circle cx="228" cy="260" r="9" fill="#2D6A4F"/>
          <circle cx="205" cy="258" r="9" fill="#2D6A4F"/>
          {/* Frond 1 - drooping left far */}
          <path d="M220 255 Q160 200 60 240 Q100 190 160 180 Q190 190 220 255Z" fill="#2D6A4F"/>
          <path d="M220 255 Q155 210 80 235" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 2 - drooping left */}
          <path d="M218 250 Q140 170 30 190 Q90 140 155 145 Q190 165 218 250Z" fill="#2D6A4F"/>
          <path d="M218 250 Q145 175 55 188" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 3 - up-left */}
          <path d="M215 248 Q160 130 80 80 Q130 100 170 130 Q200 170 215 248Z" fill="#2D6A4F"/>
          <path d="M215 248 Q165 140 95 90" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 4 - top center */}
          <path d="M220 245 Q215 130 230 50 Q240 100 240 160 Q235 210 220 245Z" fill="#2D6A4F"/>
          <path d="M220 245 Q218 145 228 65" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 5 - up-right */}
          <path d="M225 248 Q290 130 370 90 Q320 115 280 145 Q245 180 225 248Z" fill="#2D6A4F"/>
          <path d="M225 248 Q285 140 355 95" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 6 - drooping right */}
          <path d="M228 250 Q310 175 420 200 Q360 148 295 150 Q255 168 228 250Z" fill="#2D6A4F"/>
          <path d="M228 250 Q305 180 395 198" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 7 - drooping right far */}
          <path d="M225 255 Q300 210 400 260 Q350 200 290 195 Q255 205 225 255Z" fill="#2D6A4F"/>
          <path d="M225 255 Q295 215 380 250" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 8 - drooping back-left */}
          <path d="M215 252 Q120 220 20 280 Q80 225 140 215 Q180 220 215 252Z" fill="#52B788" opacity="0.6"/>
          {/* Frond 9 - drooping back-right */}
          <path d="M230 252 Q330 225 440 290 Q370 230 310 220 Q265 222 230 252Z" fill="#52B788" opacity="0.6"/>
        </svg>

        {/* Right coconut palm tree */}
        <svg className="absolute right-0 bottom-0 w-[260px] md:w-[370px] h-auto opacity-[0.12] pointer-events-none" viewBox="0 0 500 900" fill="none" xmlns="http://www.w3.org/2000/svg">
          {/* Trunk - curved, leaning slightly left */}
          <path d="M340 900 Q350 680 330 540 Q315 430 300 340 Q290 280 280 230" stroke="#2D6A4F" strokeWidth="20" strokeLinecap="round" fill="none"/>
          {/* Trunk texture marks */}
          <path d="M342 820Q325 815 315 820M340 750Q323 745 313 750M337 680Q320 675 310 680M332 610Q315 605 305 610M325 540Q308 535 298 540M318 470Q301 465 291 470M310 400Q293 395 283 400M302 340Q288 335 278 340M295 290Q281 285 271 290" stroke="#2D6A4F" strokeWidth="2" opacity="0.5"/>
          {/* Coconut cluster */}
          <circle cx="285" cy="238" r="9" fill="#2D6A4F"/>
          <circle cx="273" cy="230" r="8" fill="#2D6A4F"/>
          <circle cx="295" cy="230" r="8" fill="#2D6A4F"/>
          {/* Frond 1 - drooping right far */}
          <path d="M280 225 Q340 175 440 210 Q400 165 340 155 Q310 162 280 225Z" fill="#2D6A4F"/>
          <path d="M280 225 Q340 178 415 205" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 2 - drooping right */}
          <path d="M282 220 Q360 145 470 160 Q410 115 345 115 Q310 130 282 220Z" fill="#2D6A4F"/>
          <path d="M282 220 Q358 150 445 158" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 3 - up-right */}
          <path d="M285 218 Q340 105 420 55 Q370 75 330 105 Q300 140 285 218Z" fill="#2D6A4F"/>
          <path d="M285 218 Q335 110 405 60" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 4 - top center */}
          <path d="M280 215 Q275 105 270 25 Q262 75 260 135 Q265 180 280 215Z" fill="#2D6A4F"/>
          <path d="M280 215 Q276 115 271 40" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 5 - up-left */}
          <path d="M275 218 Q210 105 130 60 Q180 80 220 110 Q255 145 275 218Z" fill="#2D6A4F"/>
          <path d="M275 218 Q215 110 145 68" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 6 - drooping left */}
          <path d="M272 220 Q190 148 80 165 Q140 118 205 118 Q245 132 272 220Z" fill="#2D6A4F"/>
          <path d="M272 220 Q195 152 105 163" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 7 - drooping left far */}
          <path d="M275 225 Q200 180 100 220 Q150 170 210 165 Q245 172 275 225Z" fill="#2D6A4F"/>
          <path d="M275 225 Q205 182 120 215" stroke="#52B788" strokeWidth="1.5" fill="none"/>
          {/* Frond 8 - back fronds */}
          <path d="M285 222 Q380 195 460 255 Q395 200 335 190 Q305 192 285 222Z" fill="#52B788" opacity="0.6"/>
          <path d="M270 222 Q170 198 60 260 Q135 205 195 198 Q240 200 270 222Z" fill="#52B788" opacity="0.6"/>
        </svg>
